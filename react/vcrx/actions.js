...
import {
    ...
    LEAVE_KICK_ETC,
    LEAVE_KICK_DUPLICATE, ACCESS_MICRO
} from "./constants";
import {
    DEFAULT_SERVER_URL,
...
export function saveLogsPermisionAccessMicro(status) {
    return function (dispatch, getState) {
        let userId = getState()["vcrx"].userInfo.id;
        let roomId = getState()["vcrx"].roomInfo.idRoom;
        let data = {
            userId  : userId,
            roomId  : roomId,
            system  : MOBILE_SYSTEM,
        ...
        dispatch(saveLogInfo(data, ACCESS_MICRO));
    };
}
