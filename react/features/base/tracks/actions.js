...
import { saveLogsPermisionAccessMicro } from "../../../vcrx";
...
export function trackAdded(track) {
    return (dispatch, getState) => {
        if (track.track.kind === "audio") {
            dispatch(saveLogsPermisionAccessMicro(true));
        }
        track.on(
...
function _onCreateLocalTracksRejected({ gum }, device) {
    return dispatch => {
        ...
        if (gum.devices[0] === "audio") {
            dispatch(saveLogsPermisionAccessMicro(false));
        }
        if (gum) {
            const { error } = gum;
